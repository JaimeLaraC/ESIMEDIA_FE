...
export default function ProfilePageUser() {
  ...
  log('info', 'ProfilePageUser: Rendering for userId:', userId)
  ...
  const loadUserProfile = async () => {
    ...
    try {
      ...
      // Crear el perfil usando la información de suscripción del contexto
      const profile = mapDTOToUserProfile(userData, subscription.isPremium)
      
      log('info', 'ProfilePageUser: Datos del perfil cargados:', profile)
      setProfileData(profile)
      
    } catch (err) {
      log('error', 'ProfilePageUser: Error cargando perfil:', err)
      
      // Si es error de autorización, redirigir a página de error 401 y limpiar contexto
      if (err instanceof Error && err.message === 'UNAUTHORIZED') {
        log('warn', 'ProfilePageUser: Sesión expirada, redirigiendo a 401')
        setUser(null)
  ...
  const handleSaveProfile = async (data: { alias?: string; firstName: string; lastName: string }) => {
    try {
      log('info', 'Guardando perfil de usuario:', data)
      ...
      const updatedProfile = mapDTOToUserProfile(updatedUser, subscription.isPremium)
  ...
  // Renderizar usando el componente base ProfilePage
  return (
    <ProfilePage
