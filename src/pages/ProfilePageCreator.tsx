...
export default function ProfilePageCreator() {
  ...
  log('info', 'ProfilePageCreator: Rendering for userId:', userId)
  ...
  const loadUserProfile = async () => {
    ...
    try {
      ...
      log('info', 'ProfilePageCreator: Datos del perfil cargados:', profile)
      setProfileData(profile)
      
    } catch (err) {
      log('error', 'ProfilePageCreator: Error cargando perfil:', err)
      
      // Si es error de autorización, redirigir a página de error 401 y limpiar contexto
      if (err instanceof Error && err.message === 'UNAUTHORIZED') {
        log('warn', 'ProfilePageCreator: Sesión expirada, redirigiendo a 401')
        setUser(null)
  ...
  const handleSaveProfile = async (data: { alias?: string; firstName: string; lastName: string }) => {
    try {
      log('info', 'Guardando perfil de creador:', data)
